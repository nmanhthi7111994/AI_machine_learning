...
class StockComponents:
    ...
    def __init__(self, symbol: str, source: str = "VCI"):
        ...
        if self.source not in self.SUPPORTED_SOURCES:
            raise ValueError(f"Hiện tại chỉ có nguồn dữ liệu từ {', '.join(self.SUPPORTED_SOURCES)} được hỗ trợ chính thức bởi ABCDF.")
...
class Quote:
    ...
    def __init__(self, symbol: str, source: str = "VCI"):
        ...
        self.source = source.upper()
        if self.source not in self.SUPPORTED_SOURCES:
            raise ValueError(f"Hiện tại chỉ có nguồn dữ liệu từ {', '.join(self.SUPPORTED_SOURCES)} được hỗ trợ ABCDF.")
        self.symbol = symbol.upper()
